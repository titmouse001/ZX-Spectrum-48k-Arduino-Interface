...
bool findMarkerOptimized(long start_pos, unsigned& rle_data_length) {
	...
	constexpr size_t SEARCH_BUFFER_SIZE = FILE_READ_BUFFER_SIZE; // Use the defined file read buffer size
	...
	if (header_result < 0) {
		...
		if (header_result >= Z80_VERSION_2) {  // V2 or V3
			while (SdCardSupport::fileAvailable()) {
				...
				uint16_t compressed_length = (unsigned int)packetBuffer[FILE_READ_BUFFER_OFFSET] + (unsigned int)packetBuffer[FILE_READ_BUFFER_OFFSET + 1] * 0x100;
				...
				int page_number = packetBuffer[FILE_READ_BUFFER_OFFSET]; 
...
static RLEDecodeResult decodeRLE_core(unsigned sourceLengthLimit, unsigned& bytesWrittenToOutput, uint16_t currentAddress) {
  ...
  auto getNextByteFromFile = [&]() -> int {
    if (fileReadBufferCurrentPos >= fileReadBufferBytesAvailable) {
      ...
      int bytesJustRead = SdCardSupport::fileRead(fileReadBufferPtr, bytesToRead);
      if (bytesJustRead <= 0) { return -1; }  // Error reading or EOF 
      fileReadBufferBytesAvailable = bytesJustRead;
  ...
  int b1_int, b2_int; // Use int to check for -1 (EOF/error)... TODO: probably need to just do this kind of thing at pre-validation time!
    ...
    if (b1 == 0xED) {
      ...
      if (b2_int == -1) { // If ED is the last byte, use as data
         addByteToCommandPayloadBuffer(b1);
				 oled.print("Truncated RLE");
         break; // maybe treat as error ?!?!?   (needs to move/only exist in to validation)
      }
      ...
      if (b2 == 0xED) {
        ...
        int run_int = getNextByteFromFile();
        int value_int = getNextByteFromFile();
        if (run_int == -1 || value_int == -1) {oled.print("Unexpected RLE") ;  return RLE_ERROR_TRUNCATED_SEQUENCE; } 
...
int readZ80Header(uint8_t* v1_header, uint8_t* pc_low, uint8_t* pc_high, uint8_t* hw_mode, bool* isV1Compressed) {
    ...
    // Read into the file read buffer area
		...
		int len_lo = packetBuffer[FILE_READ_BUFFER_OFFSET];
		int len_hi = packetBuffer[FILE_READ_BUFFER_OFFSET + 1];
		unsigned ext_len = len_lo | (len_hi << 8);

        unsigned bytes_to_read_ext_header = ext_len;
        unsigned bytes_read_so_far = 0;

        while(bytes_read_so_far < bytes_to_read_ext_header) {
            uint16_t chunk_size = min(FILE_READ_BUFFER_SIZE, (bytes_to_read_ext_header - bytes_read_so_far));
...
BlockReadResult z80_readAndWriteBlock(uint8_t* page_number_out, uint16_t* uncompressed_length_out) {
	...
	compressed_length = (unsigned int)packetBuffer[FILE_READ_BUFFER_OFFSET] + (unsigned int)packetBuffer[FILE_READ_BUFFER_OFFSET + 1] * 0x100;
	...
	*page_number_out = packetBuffer[FILE_READ_BUFFER_OFFSET];
	...
	if (sna_offset == -1) {
		...
		while (bytes_consumed < compressed_length) {
			uint16_t bytes_to_read_this_chunk = min(FILE_READ_BUFFER_SIZE, (size_t)(compressed_length - bytes_consumed));
