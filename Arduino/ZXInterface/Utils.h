...
__attribute__((optimize("-Ofast"))) inline uint16_t zx_spectrum_screen_address(uint8_t x, uint8_t y) {
...
__attribute__((optimize("-Ofast"))) inline uint16_t zx_spectrum_screen_address(uint8_t y) {
...
__attribute__((optimize("-Ofast"))) inline void memsetZero(byte* b, unsigned int len) {
  for (; len != 0; len--) { *b++ = 0; }
}

__attribute__((optimize("-Ofast"))) inline void join6Bits(byte* output, uint8_t input, uint16_t bitPosition) {
