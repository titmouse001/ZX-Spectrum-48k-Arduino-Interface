...
// to-do , we dont need these cmd vars now with the 
// jump table fix in rom at 0x0D0 (just use cmd_addr with a enum)

// change these over to use enums to support cmd_addr(n)

    uint16_t command_NOP;
    ...
    // Setup jumps into jump table, here's the Z80 asm for ref:
    // mainloop: ld c,$1F  
	//           READ_PAIR_WITH_HALT h,l  ; HL = jump address
 	//           JP (hl) 
    void initialize() {
        command_NOP                = cmd_addr(0);
